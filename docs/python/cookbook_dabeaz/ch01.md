...
???+ quote "Discussion"
    ...
    :target: But **`itemgetter()`** is **faster** than `lambda` expressions.

    :target: Note that this technique can also be used for other functions that does their operations
